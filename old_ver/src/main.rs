...
fn print_usage(program: &str) {
    println!("Usage:");
    println!("  {} file1.bin file2.bin           # Compare and save diff.bdiff", program);
...
fn compare(file1_path: &str, file2_path: &str) -> std::io::Result<()> {
    ...
    let mut diff_out = File::create("diff.bdiff")?;
    ...
    println!("\n✅ Done! Saved {} difference blocks to diff.bdiff", diff_blocks);
    Ok(())
}
...
fn redo(file1_path: &str, diff_path: &str) -> std::io::Result<()> {
    ...
    let mut out = File::create("file1_diff.bdiff")?;
    out.write_all(&data)?;

    println!("✅ Patching complete. Saved to file1_diff.bdiff ({} blocks applied)", applied);
    Ok(())
}
